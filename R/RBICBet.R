...
RBICBet = function(y, X, Z, mBin = NULL, family,
  ...
  nu = weightF1(PearsonResid, c1)*UMu
  
  expNu = rep(NA, n)
  expNuUMu = rep(NA, n)
  if (family[[1]] == "poisson") {
    # Limit the number of considered observations considerably
    minValue <- pmax(0, floor(mu - c1 * (1 / theta) * mu))
    maxValue <- ceiling(mu + c1 * (1/theta) * mu)
    tol <- 1e-12
  ...
  pseuResp = eta +(nu - expNu)/pmax((expNuUMu*family$mu.eta(eta)), 10^(-12))
  
  weightsAM = diag(c(expNuUMu * weights.xz1 * family$mu.eta(eta)^2))
  ...
  if (family[[1]] == "poisson") {
    ...
    Pinv = solve((1/n*(M[1:p1,1:p1] + t(rS)%*%rS) ),diag(rep(1,p1)))
  }else if(family[[1]] == "binomial"){
  ...
  RBICBet = t(pseuResp- fit$B1 %*% beta) %*% weightsAM %*% (pseuResp- fit$B1 %*% beta) + log(n)/n* sum(diag(Pinv %*% Q[1:p1,1:p1]))
  return(list(RBICBet=RBICBet, fit=fit))
}
